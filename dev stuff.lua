...
for i, botName in ipairs(bots) do
    ...
    if mainAccount then
        -- Store the last processed message and timestamp to avoid processing duplicates
        local lastProcessedMsg = ""
        local lastProcessedTime = 0
        
        -- Create a continuous loop to monitor chat messages instead of using the Chatted event
        -- This ensures more reliable command processing as it doesn't depend on event firing
        task.spawn(function()
            while true do
                task.wait(0.1) -- Check for new messages every 0.1 seconds
                
                -- Try multiple methods to get chat messages
                local chatHistory = {}
                local mainAccountLastMessage = ""
                
                -- Method 1: Try to get chat history from TextChatService
                pcall(function()
                    local textChatService = game:GetService("TextChatService")
                    if textChatService and textChatService.ChatHistory then
                        chatHistory = textChatService.ChatHistory:GetChildren()
                    end
                end)
                
                -- Method 2: Try to get messages from legacy chat system
                if #chatHistory == 0 then
                    pcall(function()
                        local messages = game:GetService("TextService"):GetTextMessagesAsync()
                ...
                -- Method 3: Direct monitoring of main account's chat events
                -- This is a fallback that will capture any new messages during this cycle
                pcall(function()
                    if mainAccount and mainAccount.Chatted then
                        local newMsg = ""
                        local connection = mainAccount.Chatted:Connect(function(message)
                            newMsg = message
                        end)
                        task.wait(0.05) -- Brief wait to see if any messages come in
                        connection:Disconnect()
                        
                        if newMsg ~= "" and newMsg ~= mainAccountLastMessage then
                            mainAccountLastMessage = newMsg
                            table.insert(chatHistory, {TextSource = {UserId = mainAccount.UserId}, Text = newMsg, Timestamp = tick()})
                        end
                    end
                end)
                
                -- Method 4: Directly check chat GUI for messages
                pcall(function()
                    -- Try to find chat GUI in PlayerGui
                    local playerGui = game:GetService("Players").LocalPlayer:FindFirstChild("PlayerGui")
                    if playerGui then
                        -- Look for chat GUI in different possible locations
                        local chatGui = playerGui:FindFirstChild("Chat") or 
                                       playerGui:FindFirstChild("ChatGui") or
                                       playerGui:FindFirstChild("ChatFrame")
                        
                        if chatGui then
                            -- Find chat messages container
                            local chatContainer = chatGui:FindFirstChild("Frame") or 
                                                chatGui:FindFirstChild("ChatChannelParentFrame") or
                                                chatGui:FindFirstChild("ChatBarParentFrame")
                            
                            -- Recursively search for text labels that might contain chat messages
                            if chatContainer then
                                local function searchForChatMessages(parent)
                                    for _, child in pairs(parent:GetChildren()) do
                                        if child:IsA("TextLabel") or child:IsA("TextButton") then
                                            local text = child.Text
                                            if text and text ~= "" then
                                                -- Check if this looks like a chat message from main account
                                                if string.find(text, mainAccount.Name) or string.find(text, mainAccount.DisplayName) then
                                                    -- Extract just the message part (after username)
                                                    local message = string.match(text, ": (.+)$")
                                                    if message and message ~= mainAccountLastMessage then
                                                        mainAccountLastMessage = message
                                                        table.insert(chatHistory, {TextSource = {UserId = mainAccount.UserId}, Text = message, Timestamp = tick()})
                                                    end
                                                end
                                            end
                                        end
                                        ...
                                        if #child:GetChildren() > 0 then
                                            searchForChatMessages(child)
                                        end
                                    end
                                end
                                
                                searchForChatMessages(chatContainer)
                            end
                        end
                    end
                end)
                
                -- Method 5: Monitor chat remote events directly
                -- This is the most direct method as it intercepts the actual network traffic for chat
                pcall(function()
                    -- Check for legacy chat system remote events
                    local replicatedStorage = game:GetService("ReplicatedStorage")
                    local defaultChatSystem = replicatedStorage:FindFirstChild("DefaultChatSystemChatEvents")
                    
                    if defaultChatSystem then
                        local onMessageEvent = defaultChatSystem:FindFirstChild("OnMessageDoneFiltering") or
                                             defaultChatSystem:FindFirstChild("OnNewMessage")
                        
                        if onMessageEvent and onMessageEvent:IsA("RemoteEvent") then
                            -- Create a temporary connection to capture any new messages
                            local newMessage = nil
                            local connection = onMessageEvent.OnClientEvent:Connect(function(messageData)
                                if messageData and messageData.FromSpeaker == mainAccount.Name then
                            ...
                            if newMessage and newMessage ~= mainAccountLastMessage then
                                mainAccountLastMessage = newMessage
                                table.insert(chatHistory, {TextSource = {UserId = mainAccount.UserId}, Text = newMessage, Timestamp = tick()})
                            end
                        end
                    end
                end)
                
                -- Process the most recent message from the main account if it's new
                for _, chatMessage in pairs(chatHistory) do
                    local speaker = chatMessage.TextSource
                    local msg = chatMessage.Text or ""
                    local timestamp = chatMessage.Timestamp or tick()
                    local isFromMainAccount = false
                    
                    -- Check if this message is from the main account using various methods
                    if speaker and speaker.UserId and speaker.UserId == mainAccount.UserId then
                        isFromMainAccount = true
                    elseif type(msg) == "string" then
                        -- Check if message starts with username format
                        if string.match(msg, "^" .. mainAccount.Name .. ": ") or 
                           string.match(msg, "^" .. mainAccount.DisplayName .. ": ") then
                            -- Extract the actual message content
                            msg = string.match(msg, ": (.+)$") or msg
                            isFromMainAccount = true
                        end
                    end
                    
                    -- Process message if it's from main account and is new
                    if isFromMainAccount and (msg ~= lastProcessedMsg or timestamp > lastProcessedTime + 1) then
                        -- Update the last processed message info
